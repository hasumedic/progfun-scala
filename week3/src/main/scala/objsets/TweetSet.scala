...
class Empty extends TweetSet {
  ...
  override def mostRetweetedAcc(t: Tweet): Tweet = t

  override def descendingByRetweetAcc(list: TweetList): TweetList = list
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /**
   * We include the element in this into that, and we do this recursively for the left and right subtrees
   */
  ...
  override def mostRetweetedAcc(t: Tweet): Tweet = {
    if (elem.retweets > t.retweets) right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else right.mostRetweetedAcc(left.mostRetweetedAcc(t))
  }

  override def descendingByRetweetAcc(list: TweetList): TweetList = {
    new Cons(this.mostRetweeted, this.remove(this.mostRetweeted).descendingByRetweetAcc(list))
  }
}
