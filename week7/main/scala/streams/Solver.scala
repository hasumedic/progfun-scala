...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.empty
      ...
      val more = newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), explored + initial.head._1)
      more ++ from(initial.tail ++ more, explored + initial.head._1)
    }
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (goal, moves) #:: tail => moves.reverse
  }
}
